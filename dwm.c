...
runautostart(void)
{
    ...
    pathpfx = ecalloc(1, strlen(home) + strlen(dwmdir) + 2);
...
tile(Monitor *m)
{
	...
	if (n > m->nmaster)
		...
		mw = m->ww - (2 * m->gappx);
	for (i = 0, my = ty = (2 * m->gappx), c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			h = (m->wh - my) / (MIN(n, m->nmaster) - i) - (2 * m->gappx);
			resize(c, m->wx + (2 * m->gappx), m->wy + my, mw - (2*c->bw) - (2 * m->gappx), h - (2*c->bw), 0);
			if (my + HEIGHT(c) < m->wh)
				my += HEIGHT(c) + (2 * m->gappx);
		} else {
			h = (m->wh - ty) / (n - i) - (2 * m->gappx);
			resize(c, m->wx + mw + (2 * m->gappx), m->wy + ty, m->ww - mw - (2*c->bw) - 2*(2 * m->gappx), h - (2*c->bw), 0);
			if (ty + HEIGHT(c) < m->wh)
...
{
	unsigned int i = 0, n = 0, h = 0, mw = 0, mx = 0, my = 0, oty = 0, ety = 0, tw = 0;
	Client *c = NULL;
    ...
    /** 
     *  n           - qty of clients
     *  m->nmaster  - qty of master windows
     *  m->mfact    - factor of master area size [0.05..0.95]
     *
     *  m->ww       - window width
     *  m->wh       - window height
     *  mw          - master width
     *  tw          - stack width
     *  mx          - master area X coordinate
     *  my          - master area Y coordinate
     *  
     */
	if (n > m->nmaster) {
    ...
    /**
     *  c       - client
     *  c->bw   - client's border width
     */
    /** Go over all windows and resizing these */
...
centeredfloatingmaster(Monitor *m)
{
	...
	if (i < m->nmaster) {
		...
		resize(c, m->wx + mx + m->gappx, m->wy + my + m->gappx, w - (2*c->bw) - (2 * m->gappx),
		       mh - (2*c->bw) - (2 * m->gappx), 0);
		mx += WIDTH(c);
	} else {
